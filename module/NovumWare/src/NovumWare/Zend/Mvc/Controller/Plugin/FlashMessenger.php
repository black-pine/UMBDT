<?php
namespace NovumWare\Zend\Mvc\Controller\Plugin;
 ...
 */
class FlashMessenger extends \Zend\Mvc\Controller\Plugin\AbstractPlugin
{
	/**
	...
	public function getContainer()
	{
		...
		$this->container = new Container(\Application\Constants\ApplicationConstants::SESSION_NAME.'_nwFlashMessenger', $manager);

//		$this->container->getManager()->destroy(); // to clean the $_SESSION
...
//	}
	...
	public function getAllMessages() {
		...
		foreach ($this->messages as $namespace => $namespaceMessages) $messages[$namespace] = $namespaceMessages->toArray();

		$container = $this->getContainer();
		foreach ($container as $namespace => $namespaceMessages) $messages[$namespace] = (isset($messages[$namespace])) ? array_merge($messages[$namespace], $namespaceMessages->toArray()) : $namespaceMessages->toArray();

		$this->clearAllMessages();
		$this->clearAllCurrentMessages();
...
//	}
	...
	public function clearAllMessages() {
		foreach ($this->messages as $namespace => $messages) unset($this->messages[$namespace]);
	}
...
//	}
	 ...
	 * @return void
	 */
	public function clearAllCurrentMessages() {
		$container = $this->getContainer();
		foreach ($container as $namespace => $messages) unset($container->{$namespace});
	}
